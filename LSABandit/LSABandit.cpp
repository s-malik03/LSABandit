...
bool AcquireOplock()
{
    ...
    {
        Log(Error, OBFUSCATE("CreateFile for oplock failed with GLE %u"), GetLastError());
        return false;
    }
    ...
    {
        Log(Error, OBFUSCATE("DeviceIoControl for oplock succeeded when it should not have"));
        CloseHandle(hFile);
    ...
    {
        Log(Error, OBFUSCATE("DeviceIoControl for oplock failed with unexpected GLE %u"), GetLastError());
        CloseHandle(hFile);
        CloseHandle(ovl.hEvent);
        return false;
    }

    Log(Debug, OBFUSCATE("Acquired exclusive oplock to file: %ws"), gpOplockFile);
